...
#import "SHAppStatus.h" //for check streethawkEnabled
...
{
    static SHLocationManager *sharedLocationManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedLocationManager = [[SHLocationManager alloc] init];
    });    
...
+ (BOOL)locationServiceEnabledForApp:(BOOL)allowNotDetermined
{
    ...
    if (!StreetHawk.isLocationServiceEnabled/*code allow*/ || ![CLLocationManager locationServicesEnabled]/**Global location service is enabled.*/)
    {
        return NO;
    }
    BOOL isEnabled;
...
{
    if ([SHLocationManager locationServiceEnabledForApp:NO])  //If location service disabled, this local value is meaningless.
    {
...
{
    if ([SHAppStatus sharedInstance].streethawkEnabled && StreetHawk.isLocationServiceEnabled/*bellow promote location permission, do it only when StreetHawk allows*/ && [CLLocationManager authorizationStatus] == kCLAuthorizationStatusNotDetermined)
    {
        NSString *locationAlwaysStr = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSLocationAlwaysUsageDescription"];
...
- (BOOL)startMonitorGeoLocationStandard:(BOOL)standard
{
    ...
    if (!StreetHawk.isLocationServiceEnabled)
    {
    ...
    {
        if ([SHLocationManager locationServiceEnabledForApp:YES/*for first time promote permission dialog*/])
        {
            ...
            if (standard)
            {
                ...
                SHLog(@"LocationManager Action: Start standard location update.");
            ...
            else
            {
                ...
                {
                    SHLog(@"LocationManager Action: Start significant location update.");
...
- (void)stopMonitorGeoLocation
{
    ...
    if (self.geolocationMonitorState != SHGeoLocationMonitorState_Stopped)
    {
        ...
        {
            SHLog(@"LocationManager Action: Stop standard location update.");
        ...
        {
            SHLog(@"LocationManager Action: Stop significant location update.");
...
- (BOOL)startMonitorRegion:(CLRegion *)region
{
    ...
    [self requestPermissionSinceiOS8]; //request before action, it simply return if not suitable.
    BOOL isMonitorRegionAvailable = [SHLocationManager locationServiceEnabledForApp:NO];
    ...
    if (sameIdRegion != nil)
    {
        ...
        {
            SHLog(@"Warning: same identifier region %@ monitored. Add new %@ will remove the already monitored one.", sameIdRegion, region);
        }
    }
    SHLog(@"LocationManager Action: Start monitor region %@.", region);
...
- (void)stopMonitorRegion:(CLRegion *)region
{
    ...
    if (isFound)
    {
        SHLog(@"LocationManager Action: Stop monitor region %@.", region);
...
- (BOOL)startRangeiBeaconRegion:(CLBeaconRegion *)iBeaconRegion
{
    ...
    if (!isFound)
    {
        SHLog(@"LocationManager Action: Start range region %@.", iBeaconRegion);
...
- (void)stopRangeiBeaconRegion:(CLBeaconRegion *)iBeaconRegion
{
    ...
    if (isFound)
    {
        SHLog(@"LocationManager Action: Stop range region %@.", iBeaconRegion);
...
- (void)locationManager:(CLLocationManager *)manager didFailWithError:(NSError *)error
{
    ...
    {
        SHLog(@"LocationManager Delegate: Update Failed: %@", [error localizedDescription]);
    }
...
- (void)locationManager:(CLLocationManager *)manager didEnterRegion:(CLRegion *)region
{
    ...
    SHLog(@"LocationManager Delegate: Enter Region: %@", region);
    NSDictionary *userInfo = @{SHLMNotification_kRegion: region};
...
- (void)locationManager:(CLLocationManager *)manager didExitRegion:(CLRegion *)region
{
    ...
    SHLog(@"LocationManager Delegate: Exit Region: %@", region);
    NSDictionary *userInfo = @{SHLMNotification_kRegion: region};
...
- (void)locationManager:(CLLocationManager *)manager didStartMonitoringForRegion:(CLRegion *)region
{
    ...
    SHLog(@"LocationManager Delegate: Monitoring started for region: %@", region);
    NSDictionary *userInfo = @{SHLMNotification_kRegion: region};
...
- (void)locationManager:(CLLocationManager *)manager monitoringDidFailForRegion:(CLRegion *)region withError:(NSError *)error
{
    ...
    SHLog(@"LocationManager Delegate: Monitoring Failed for Region(%@): %@", region, [error localizedDescription]);
...
- (void)locationManager:(CLLocationManager *)manager didDetermineState:(CLRegionState)state forRegion:(CLRegion *)region
{
    ...
    SHLog(@"LocationManager Delegate: Determine State %@ for Region %@", strState, region);
...
- (void)locationManager:(CLLocationManager *)manager didRangeBeacons:(NSArray *)beacons inRegion:(CLBeaconRegion *)region
{
    ...
    SHLog(@"LocationManager Delegate: did range beacons: %@ for region: %@.", beacons, region);
...
- (void)locationManager:(CLLocationManager *)manager rangingBeaconsDidFailForRegion:(CLBeaconRegion *)region withError:(NSError *)error
{
    ...
    SHLog(@"LocationManager Delegate: range location Failed for Region(%@): %@", region, [error localizedDescription]);
    NSDictionary *userInfo = @{SHLMNotification_kRegion: region, SHLMNotification_kError: error};
