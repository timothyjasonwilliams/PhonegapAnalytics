...
#define SHMBMainThreadAssert() NSAssert([NSThread isMainThread], @"SHMBProgressHUD needs to be accessed on the main thread.");

CGFloat const SHMBProgressMaxOffset = 1000000.f;

static const CGFloat SHMBDefaultPadding = 4.f;
static const CGFloat SHMBDefaultLabelFontSize = 16.f;
static const CGFloat SHMBDefaultDetailsLabelFontSize = 12.f;


@interface SHMBProgressHUD () {
    // Deprecated
...
@interface SHMBProgressHUDRoundedButton : UIButton
@end


@implementation SHMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    SHMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    SHMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (SHMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (SHMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = SHMBProgressHUDAnimationFade;
    _mode = SHMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(SHMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == SHMBProgressHUDAnimationZoom) {
        type = animatingIn ? SHMBProgressHUDAnimationZoomIn : SHMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == SHMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == SHMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == SHMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == SHMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    SHMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<SHMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    SHMBBackgroundView *backgroundView = [[SHMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = SHMBProgressHUDBackgroundStyleSolidColor;
    ...
    SHMBBackgroundView *bezelView = [SHMBBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:SHMBDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:SHMBDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [SHMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:SHMBDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[SHMBRoundProgressView class]];

    SHMBProgressHUDMode mode = self.mode;
    if (mode == SHMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == SHMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[SHMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SHMBProgressHUDModeDeterminate || mode == SHMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[SHMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == SHMBProgressHUDModeAnnularDeterminate) {
            [(SHMBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == SHMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SHMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[SHMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[SHMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[SHMBRoundProgressView class]]) {
        SHMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SHMBRoundProgressView appearanceWhenContainedIn:[SHMBProgressHUD class], nil];
#else
        appearance = [SHMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[SHMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((SHMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((SHMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[SHMBBarProgressView class]]) {
        SHMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SHMBBarProgressView appearanceWhenContainedIn:[SHMBProgressHUD class], nil];
#else
        appearance = [SHMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[SHMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((SHMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((SHMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    SHMBBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? SHMBDefaultPadding : 0.f;
...
- (void)setMode:(SHMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == SHMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation SHMBRoundProgressView
...
@implementation SHMBBarProgressView
...
@implementation SHMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = SHMBProgressHUDBackgroundStyleBlur;
            ...
            _style = SHMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(SHMBProgressHUDBackgroundStyle)style {
    if (style == SHMBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = SHMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    SHMBProgressHUDBackgroundStyle style = self.style;
    if (style == SHMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == SHMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation SHMBProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [SHMBProgressHUD allHUDsForView:view];
    for (SHMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable SHMBProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    SHMBBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == SHMBProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    SHMBMainThreadAssert();
    self.backgroundView.style = SHMBProgressHUDBackgroundStyleSolidColor;
...
@implementation SHMBProgressHUDRoundedButton
