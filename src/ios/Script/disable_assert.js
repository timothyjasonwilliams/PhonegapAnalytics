	...
	fs.readFile(cordovaConfigPath, {encoding: "utf-8"}, function(errConfigRead, configContent) {
		...
		xcodeProject.parse(function(parseError) {
			...
			Object.keys(configurations).forEach(function(config) {
				...
				if (!buildSettings['OTHER_CFLAGS'] || buildSettings['OTHER_CFLAGS'] === '"$(inherited)"' || buildSettings['OTHER_CFLAGS'] === '$(inherited)') {
        		...
        		if (!Array.isArray(buildSettings['OTHER_CFLAGS'])) {
        			buildSettings['OTHER_CFLAGS'] = [buildSettings['OTHER_CFLAGS']];
        		}
        		//Finally add disable assert flags.
		        buildSettings['OTHER_CFLAGS'].push('"-DNS_BLOCK_ASSERTIONS=1"');
				buildSettings['OTHER_CFLAGS'].push('"-DNDEBUG"');
			});
